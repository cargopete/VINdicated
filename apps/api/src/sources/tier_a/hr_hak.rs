...
/// Croatia — HAK / CVH (Croatian Vehicle History)
/// eservisi.hak.hr is web-form based. No public JSON API endpoint verified.
pub struct HrHak {
...
impl VehicleSource for HrHak {
    ...
    fn name(&self) -> &'static str { "HAK/CVH (HR)" }
    ...
    async fn fetch_by_vin(&self, _vin: &str) -> Result<SourceData, SourceError> {
        Err(SourceError::Unavailable(
            "No verified public VIN API for Croatian HAK".into(),
        ))
    }
}
